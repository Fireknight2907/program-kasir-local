...
function setup(sessions, failOrder = false, menu = [{ id: 1, price: 10000, isAvailable: true }]) {
  ...
  const prisma = { async $transaction(callback) {
    let unlock, draft;
    try {
      const tx = {
        transaction: {
        ...
        orderSubmission: {
          ...
          async count({ where }) { return draft.receipts.filter(r => r.transactionId === where.transactionId && r.createdAt >= where.createdAt.gte).length; },
  ...
  return {
    ...
    state: () => ({ sessions: state.sessions, orders: state.orders }),
